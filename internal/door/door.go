...
func (c *Controller) Acquire(ctx context.Context, room model.RoomID) error {
	...
	release := func() {
		c.mu.Lock()
		defer c.mu.Unlock()
		if current, exists := c.doors[room]; exists {
			current.State = model.DoorUnlocked
			current.WaitID = ""
		}
	}
	err := waitForSettle(ctx, c.settleChannel(room), release)
	if err != nil && c.State(room) != model.DoorUnlocked {
		release()
	}
	return err
}
